...
### v0.6 (2026-02-12)
- 新增
  - 增加独立 `backend-service`（FastAPI + Jinja2 + sqlite），提供 `/web/status`、`/web/models` 与 `api/v1` 接口。
  - 新增后端执行器异步队列（并发固定 1），落地 Codex 真执行。
  - 新增模型同步接口 `POST /api/v1/models/import-astr`，仅接收 provider 元数据。
- 调整
  - V1.1 插件新增后端桥接配置项（backend_api_url/token、自动推送、执行器等待策略）。
  - `/助手 模型导出JSON` 升级为“导出 + 可选自动推送”。
  - `/助手 诊断` 增加后端探测与最后同步状态输出。
- 删除
  - 无。
- 风险评估变化
  - 新增风险：后端 API token 配置错误会导致模型同步与执行器调用失败；通过诊断错误码和审计追踪缓解。
